...
def params(named=None, unnamed=None):
	...
	def decorate(func):
		...
		def wrapper(*args, **kwargs):
			...
			len_expected, len_actual = len(unnamed), len(args)
...
@timethis
@list_arguments
@params(named=["a", "b"], unnamed=["c"])
def boo(*args, **kwargs):
	"""This function uses the 'params' decorator"""
...
@params(named=["a", "b"])
def doo(*args, **kwargs):
	"""This function also uses the 'params' decorator"""
	print("a is %s" % (kwargs["a"]))
	print("b is %s" % (kwargs["b"]))
	for arg in args:
		print(arg)
