...
def acquire_instances(ec2_conn, params):
	...
	it is fulfilled.."""
	logging.info("Acquiring instances")
	sec_groups = params["all_security_groups"]
	sec_group_ids = []
	for group in sec_groups:
		sec_group_ids.append(group.id)
	logging.info("Security group ids {0}".format(sec_group_ids))
...
def assign_ebs_volumes(ec2_conn, params):
	...
	logging.info("Snapshot: {0}".format(params["snapshot"]))
	...
	for i in instances:
		...
		for j in range(0, max_attempts):
			try:
				if j > 0:
					logging.info("Making attempt {0} of {1} to attach volume.".format(\
						j, max_attempts))
					logging.info("Placement {0}, Snapshot {1}".format(placement, snapshot))
					new_volume = ec2_conn.create_volume(100, placement,\
						snapshot=snapshot, volume_type="gp2")
					logging.info("New volume created {0}, {1}".format(new_volume.id,\
						new_volume.status))
					attach_volume_to_instance(ec2_conn, new_volume, i, params)
					break
			except Exception as err:
				j += 1
				logging.info("Exception {0}".format(err))
				logging.info("Unexpected error:", sys.exc_info()[0])
				logging.info("Attempt #{0} in 3 seconds.".format(j))
				time.sleep(3)
	logging.info("All volumes created from snapshots and mounted.")

def attach_volume_to_instance(ec2_conn, volume, instance, params):
	"""Attaches EBS volumes to EC2 instances."""
	logging.info("In attach_volume_to_instance")
	status, tries, max_tries = volume.status, 0, 60
	while (status != "available") and (tries < max_tries):
		logging.info("Volume status {0}".format(status))
		time.sleep(5)
		tries += 1
		logging.info("Updating")
		volume.update()
		status = volume.volume_state()
	logging.info("Volume available")
	my_ebs_mount = params["ebs_mount"]
	max_attempts = 60
	logging.info("Volume.id {0}, Instance.id {1}, my_ebs_mount {2}".format(\
		volume.id, instance.id, my_ebs_mount))
	status_line = "volume.id {0}, instance.id {1}, device_name {2}, "\
		"attachment_state {3}"
	for j in range(0, max_attempts):
		try:
			logging.info("Trying to attach volume")
			_ = ec2_conn.attach_volume(volume.id, instance.id, my_ebs_mount)
			logging.info("Trying to update volume stats")
			volume.update()
			logging.info("Trying to read volume state")
			status = volume.attachment_state()
			logging.info("Volume attachment state {0}".format(status))
			if status == 'attaching':
...
def create_security_group(conn, name, vpc):
	...
	try:
		...
		logging.info("Added {0}".format(my_security_group))
		return my_security_group
	except boto.exception.EC2ResponseError:
		logging.info("Unable to create {0}, aborting".format(name))
		sys.exit()
...
def customize_config_file(params, src_file):
	...
	try:
		...
		logging.info("Error copying template. {0}".format(err))
	except IOError:
		logging.info("IO Error copying template.")
...
def get_master_ip_list(params):
	...
	logging.info("master_ip_list {0}".format(params["master_ip_list"]))
...
def get_instance_lists(params):
	...
	logging.info("Masters: {0}, Hybrids: {1}, Slaves: {2}".format(master_count,\
	...
	logging.info("Masters {0}".format(params["masters"]))
	logging.info("Hybrids {0}".format(params["hybrids"]))
	logging.info("Slaves {0}".format(params["slaves"]))

def get_snapshot(ec2_conn, params):
	"""Gets the EBS snapshot from Amazon"""
	logging.info("Assigning EBS volumes")
	...
	for j in range(0, max_attempts):
		for item in ebs_mapping:
			try:
				...
				logging.info("Unexpected error:", sys.exc_info()[0])
				logging.info("Exception {0}".format(err))
				logging.info("Attempt #{0} in {1} seconds.".format(j, sleep_between_attempts))
...
def initialize():
	...
	params = None
	if len(sys.argv) != 3:
		logging.info("Supply the following arguments: config_file, cluster-name")
...
def map_block_devices(params):
	...
	bdm['/dev/sdb'] = eph0
	bdm['/dev/sdc'] = eph1
	logging.info("Map is {0}".format(bdm))
	params["bdm"] = bdm
...
def poll_for_cluster_status(params):
	...
	logging.info("Polling for cluster status green")
	...
	for j in range(0, max_attempts):
		try:
			if j > 0:
				logging.info("Making attempt {0} of {1} for cluster status.".format(j,\
					max_attempts))
				try:
					...
					logging.info("Exception while trying to make Elasticsearch connection.")
					raise "Error trying to connect to ES node."
				logging.info("Cluster is online.")
				logging.info("Attempting to poll for health.")
				...
				while (status != "green") or (number_of_nodes < target_nodes):
					...
					if result:
						...
						logging.info(status_line.format(status, number_of_nodes,\
							target_nodes))
						time.sleep(sleep_between_attempts)
					else:
						time.sleep(sleep_between_attempts)
				break
			if j >= max_attempts:
				logging.info("Error getting cluster status, aborting abnormally.")
				sys.exit()
		except Exception as err:
			j += 1
			logging.info("Exception {0}".format(err))
...
def poll_pending_instances(reservations, instance_count, params):
	...
	logging.info("Reservations {0}".format(reservations))
	...
	while count_runners < instance_count:
		...
		for i in instances:
			...
			for j in range(0, max_attempts):
				if j >= max_attempts:
					logging.info("Error updating instance state, aborting abnormally.")
					sys.exit()
				try:
					if j > 0:
						logging.info("Attempt {0} of {1} for instance update.".format(\
							j, max_attempts))
					state = i.update()
				except:
					j += 1
			...
			logging.info("IP: {0}, ID: {1}, State: {2}".format(private_ip_address,\
				instance_id, state))
		...
		if count_runners < instance_count:
			logging.info("Waiting on {0} instances...".format(pending_instances))
			time.sleep(10)
	logging.info("All instances running.")
...
def run_ssh_commands(instance_ip_address, params, command_list, login="root"):
	...
	try:
		...
		logging.info("Exception trying to ssh into host {0}".format(err))
	...
	for item in shell_commands:
		...
		if my_stdout != "[]":
			logging.info(my_stdout)
		if my_stderr != "[]":
			logging.info(my_stderr)
	stdout.close()
	stderr.close()
	ssh.close()
	return all_stdout
...
def start():
	...
	logging.info("Creating Elasticsearch repository... ", end="")
	...
	os.system(command)
	logging.info("done!")
	logging.info("Congratulations! Your cluster is fully operational!")
#Main program
