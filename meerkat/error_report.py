...
def begin_processing_loop(some_container):
	...
	pending_list = []
	error_list = []
	count_pending, count_error, count_src = 0, 0, 0
		...
		if src_s3_path_regex.search(k.key):
			...
			if file_name in completed:
				...
				if ratio >= 1.8:
					...
					#print("{0} - Has an error.".format(file_name))
					pending_list.append(k)
					error_list.append(file_name)
					count_error += 1
			else:
				#print("{0} - Not completed.".format(file_name))
				pending_list.append(k)
				count_pending += 1
	print("Pending, Error, Target: {0}, {1}, {2}".format(count_pending, count_error, count_src))

	for m in dst_bucket.list():
