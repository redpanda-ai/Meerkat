...
from meerkat.classification.tensorflow_cnn import (
	validate_config, get_tensor, string_to_tensor)
from meerkat.classification.ensemble_cnns import build_graph
...
def get_ensemble_cnns_by_path(model1, model2, model3, label_map_path, gpu_mem_fraction=False):
	"""Load a tensorFlow module by name"""
	...
	config = load_params(config_path)
	config["label_map"] = label_map_path
	config["model_path_1"] = model1
	config["model_path_2"] = model2
	config["model_path_3"] = model3
	config = validate_config(config)
	...
	saver1.restore(sess, config["model_path_1"])
	model1 = get_tensor(graph, "model1/model:0")
	saver2.restore(sess, config["model_path_2"])
	model2 = get_tensor(graph, "model2/model:0")
	saver3.restore(sess, config["model_path_3"])
	...
	def apply_cnn(trans, doc_key="description", label_key="CNN", label_only=True):
		...
		labels = np.argmax(ensemble_output, 1) + 1
