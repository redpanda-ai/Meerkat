...
def ensemble_evaluate_testset(config, graph, sess, model1, model2, model3, test):
...
def bias_variable(shape, flat_input_shape, name):
	"""Initialize biases"""
	stdv = 1 / math.sqrt(flat_input_shape)
	bias = tf.Variable(tf.random_uniform(shape, minval=-stdv, maxval=stdv), name="B")
	# bias = tf.get_variable(name, initializer=tf.random_uniform(shape, minval=-stdv, maxval=stdv))
	return bias

def weight_variable(config, shape, name):
	"""Initialize weights"""
	weight = tf.Variable(tf.mul(tf.random_normal(shape), config["randomize"]), name="W")
	# weight = tf.get_variable(name, initializer=tf.mul(tf.random_normal(shape), config["randomize"]))
	return weight
...
def build_graph(config):
	...
	with graph.as_default():
		...
		def model(data, name, train=False):
			...
			w_conv1 = weight_variable(config, [1, 7, alphabet_length, 256], "w1")
			b_conv1 = bias_variable([256], 7 * alphabet_length, "b1")

			w_conv2 = weight_variable(config, [1, 7, 256, 256], "w2")
			b_conv2 = bias_variable([256], 7 * 256, "b2")

			w_conv3 = weight_variable(config, [1, 3, 256, 256], "w3")
			b_conv3 = bias_variable([256], 3 * 256, "b3")

			w_conv4 = weight_variable(config, [1, 3, 256, 256], "w4")
			b_conv4 = bias_variable([256], 3 * 256, "b4")

			w_conv5 = weight_variable(config, [1, 3, 256, 256], "w5")
			b_conv5 = bias_variable([256], 3 * 256, "b5")

			w_fc1 = weight_variable(config, [reshape, 1024], "wfc1")
			b_fc1 = bias_variable([1024], reshape, "bfc1")

			w_fc2 = weight_variable(config, [1024, num_labels], "wfc2")
			b_fc2 = bias_variable([num_labels], 1024, "bfc2")
			...
			h_fc2_nochange = tf.matmul(h_fc1_nochange, w_fc2) + b_fc2
			softmax_nochange = tf.nn.softmax(h_fc2_nochange, name="model")
			return softmax, network, softmax_nochange

		with tf.variable_scope("model1") as model1:
			softmax1, network1, cnn1 = model(trans_placeholder, "network", train=True)
		with tf.variable_scope("model2") as model2:
			softmax2, network2, cnn2 = model(trans_placeholder, "network", train=True)
		with tf.variable_scope("model3") as model3:
			softmax3, network3, cnn3 = model(trans_placeholder, "network", train=True)

		ensemble = (softmax1 + softmax2 + softmax3)/3.0
		loss1 = tf.neg(tf.reduce_mean(tf.reduce_sum(network1 * labels_placeholder, 1)) + 0.5 * tf.reduce_mean(tf.reduce_sum((ensemble - softmax1) ** 2, 1)), name="loss1")
		loss2 = tf.neg(tf.reduce_mean(tf.reduce_sum(network2 * labels_placeholder, 1)) + 0.5 * tf.reduce_mean(tf.reduce_sum((ensemble - softmax2) ** 2, 1)), name="loss2")
		loss3 = tf.neg(tf.reduce_mean(tf.reduce_sum(network3 * labels_placeholder, 1)) + 0.5 * tf.reduce_mean(tf.reduce_sum((ensemble - softmax3) ** 2, 1)), name="loss3")
		optimizer1 = tf.train.MomentumOptimizer(learning_rate, 0.9).minimize(loss1, name="optimizer1", var_list=[x for x in tf.all_variables() if x.name.startswith("model1")])
		optimizer2 = tf.train.MomentumOptimizer(learning_rate, 0.9).minimize(loss2, name="optimizer2", var_list=[x for x in tf.all_variables() if x.name.startswith("model2")])
		optimizer3 = tf.train.MomentumOptimizer(learning_rate, 0.9).minimize(loss3, name="optimizer3", var_list=[x for x in tf.all_variables() if x.name.startswith("model3")])

		saver1 = tf.train.Saver([x for x in tf.all_variables() if x.name.startswith("model1")])
		saver2 = tf.train.Saver([x for x in tf.all_variables() if x.name.startswith("model2")])
		saver3 = tf.train.Saver([x for x in tf.all_variables() if x.name.startswith("model3")])

	return graph, saver1, saver2, saver3

def train_model(config, graph, sess, saver1, saver2, saver3):
	"""Train the model"""
	...
	for step in range(num_eras):
		...
		if step % logging_interval == 0:
			loss1 = sess.run(get_tensor(graph, "loss1:0"), feed_dict=feed_dict)
			loss2 = sess.run(get_tensor(graph, "loss2:0"), feed_dict=feed_dict)
			loss3 = sess.run(get_tensor(graph, "loss3:0"), feed_dict=feed_dict)
			logging.info("Train loss1 at epoch {0:>8}: {1:3.7f}".format(step + 1, loss1))
			logging.info("Train loss2 at epoch {0:>8}: {1:3.7f}".format(step + 1, loss2))
			logging.info("Train loss3 at epoch {0:>8}: {1:3.7f}".format(step + 1, loss3))
		...
		if step != 0 and step % epochs == 0:
			...
			#Evaluate Model
			model1 = get_tensor(graph, "model1/model:0")
			model2 = get_tensor(graph, "model2/model:0")
			model3 = get_tensor(graph, "model3/model:0")
			learning_rate = get_variable(graph, "lr:0")
			predictions1 = sess.run(model1, feed_dict=feed_dict)
			predictions2 = sess.run(model2, feed_dict=feed_dict)
			predictions3 = sess.run(model3, feed_dict=feed_dict)
			logging.info("Testing for era %d" % (step / epochs))
			logging.info("Learning rate at epoch %d: %g" % (step + 1, sess.run(learning_rate)))
			logging.info("Minibatch accuracy: %.1f%%" % accuracy(predictions1, labels))
			logging.info("Minibatch accuracy: %.1f%%" % accuracy(predictions2, labels))
			logging.info("Minibatch accuracy: %.1f%%" % accuracy(predictions3, labels))
			...
			ensemble_accuracy = ensemble_evaluate_testset(config, graph, sess, model1, model2, model3, test)

			# Save Checkpoint
			current_era = int(step / epochs)
			save_path1 = saver1.save(sess, save_dir + "era_" + str(current_era) + "_model1.ckpt")
			save_path2 = saver2.save(sess, save_dir + "era_" + str(current_era) + "_model2.ckpt")
			save_path3 = saver3.save(sess, save_dir + "era_" + str(current_era) + "_model3.ckpt")
			logging.info("Checkpoint saved in file: %s" % save_path1)
			logging.info("Checkpoint saved in file: %s" % save_path2)
			logging.info("Checkpoint saved in file: %s" % save_path3)
			checkpoints[current_era] = save_path1
...
def run_session(config, graph, saver1, saver2, saver3):
	"""Run Session"""
	...
	with tf.Session(graph=graph) as sess:
		...
		if mode == "train":
			_ = train_model(config, graph, sess, saver1, saver2, saver3)
		elif mode == "test":
...
def run_from_command_line():
	...
	graph, saver1, saver2, saver3 = build_graph(config)
	run_session(config, graph, saver1, saver2, saver3)
