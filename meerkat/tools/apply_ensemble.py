...
#################### USAGE ##########################
"""
python3 -m meerkat.tools.apply_ensemble \
-model1 <path_to_classifier> \
-model2 <path_to_classifier> \
-model3 <path_to_classifier> \
...
from meerkat.classification.lua_bridge_ensemble import get_ensemble_by_path
...
accu1 = input("Model 1 accuracy:\n")
accu2 = input("Model 2 accuracy:\n")
accu3 = input("Model 3 accuracy:\n")

confusion_matrix = [[0 for i in range(num_labels + 1)] for j in range(num_labels)]
classifier = get_ensemble_by_path(args.model1, args.model2, args.model3, dict_path=label_map, accuracy = [float(accu1), float(accu2), float(accu3)])

# Prepare for data saving
path = 'data/ensemble_stats/'
os.makedirs(path, exist_ok=True)
write_mislabeled = get_write_func(path + "ensemble_mislabeled.csv",
	['TRANSACTION_DESCRIPTION', 'ACTUAL', 'PREDICTED'])
write_correct = get_write_func(path + "ensemble_correct.csv",
	['TRANSACTION_DESCRIPTION', 'ACTUAL'])
write_unpredicted = get_write_func(path + "ensemble_unpredicted.csv",
	["TRANSACTION_DESCRIPTION", 'ACTUAL'])
write_needs_hand_labeling = get_write_func(path + "ensemble_need_labeling.csv",
...
stat.to_csv('data/ensemble_stats/ensemble_stat.csv', index=False)
cm.to_csv('data/ensemble_stats/ensemble_con_Matrix.csv')
