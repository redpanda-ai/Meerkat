...
def initialize():
	...
	if len(sys.argv) != 2:
		raise TypeError(msg="Incorrect number of arguments", expr=None)
...
def get_pending_list():
	"""gets a list of pending keys"""
	lists = []
	my_key = re.compile(".*/([^/]+)")
	#logging.critical("-> {0}".format((PARAMS["S3"])))
	for s3_dir in PARAMS["S3"]:
		keys = [j.key for j in s3_dir["s3_objects"]]
		keys = [my_key.search(k).group(1) for k in keys if my_key.search(k)]
		lists.append(keys)
...
def sort_the_file(my_file):
	...
	with gzip.open(my_file, 'rt') as file_one:
		...
		for row in csv_reader:
			...
			col = []
			for values in my_filter:
				col.append(row[values])
			line = ".".join(col)
			if first_line:
	...
	return list(my_header.values())[0], \
				OrderedDict(sorted(my_map.items(), key=lambda t: t[0])), \
				count
...
def get_columns(my_data, filter_name):
	...
	try:
		...
		logging.critical("Detected an error, skipping")
	return result
...
def merge_the_files(args, expected_lines, remainder):
	...
	bytes_written = key.set_contents_from_filename(merged_file, \
												   encrypt_key=True, \
												   replace=True)
...
def process_pending_list():
	...
	index = 0
	for s3_dir in PARAMS["S3"]:
		keys = [(j, my_key.search(j.key).group(1)) \
