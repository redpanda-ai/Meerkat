...
from meerkat.various_tools import (safely_remove_file, load_params, get_s3_connection)
...
def save_df(df, file_name, columns):
	"""Save a dataframe"""
	df.to_csv(file_name, columns=columns, sep="|", mode="w", quoting=csv.QUOTE_NONE,
		 encoding="utf-8", index=False, index_label=False)

def load_df(file_name, dtypes):
	"""Load a dataframe"""
	return pd.read_csv(file_name, na_filter=False, dtype=dtypes, quoting=csv.QUOTE_NONE,
...
def run_from_command_line():
	...
	columns = ["DESCRIPTION_UNMASKED", "DESCRIPTION", "GOOD_DESCRIPTION",


	"TRANSACTION_DATE", "UNIQUE_TRANSACTION_ID", "AMOUNT", "UNIQUE_MEM_ID", "TYPE"]
	...
	for i, item in enumerate(files):
		...
		try:
			...
			reader = pd.read_csv(file_name, na_filter=False, chunksize=500000,
			...
			for df in reader:
				...
				for merchant, merchant_df in groups.items():
					...
					if merchant == "":
						...
						if first_null_chunk:
							sampled_df.to_csv(merchant_file_name, columns=columns, sep="|",
								mode="a", encoding="utf-8", index=False, index_label=False)
							first_null_chunk = False
						else: 
							sampled_df.to_csv(merchant_file_name, header=False, columns=columns,
					...
					elif merchant_count[merchant] < sample_size:
						output_df = load_df(merchant_file_name, dtypes)		

					# Fill Reservoirs
					if merchant_count[merchant] < sample_size:
						o_len = len(output_df)
						m_len = len(merchant_df)
						if o_len + m_len <= sample_size:
							...
							rowNum = sample_size - o_len
							output_df = output_df.append(merchant_df.iloc[0:rowNum],
								ignore_index=False)
							merchant_count[merchant] += rowNum
							merchant_df = merchant_df.iloc[rowNum+1:m_len-1]
							save_df(output_df, merchant_file_name, columns)
					...
					for k in merchant_df.index:
						...
						rand = random.random()
						if rand < sample_size / merchant_count[merchant]:
					...
					if len(rows_to_add) > 0:
						...
						# Replace Rows
						output_df.iloc[np.random.choice(range(sample_size), len(rows_to_add))]\
							= merchant_df.loc[rows_to_add].values
