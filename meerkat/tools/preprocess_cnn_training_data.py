...
def get_lookup_dicts(input_dataframe):
	...
	histogram_dataframe = \
	input_dataframe.GOOD_DESCRIPTION.value_counts().head(600).to_frame()
	# Save room for the null class
	offset = 1
	# Give each non-NULL class a number starting at 2
	histogram_dataframe['INDEX'] = \
	list(range(offset, len(histogram_dataframe.index) + offset))
	...
	with open("lookup.json", 'w') as outfile:
		...
		(lookup, sort_keys=True, indent=4, separators=(',', ': ')))
	#Write reverse_lookup.json out to a file
	with open("reverse_lookup.json", 'w') as outfile:
		outfile.write(json.dumps\
		(reverse_lookup, sort_keys=True, indent=4, separators=(',', ': ')))
