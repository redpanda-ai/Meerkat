...
curl -XDELETE s01:9200/ex_index
echo -e "\nCreating a new type with a geo-shape mapping"
curl -XPUT s01:9200/ex_index -d '{
	"mappings": {
		"ex_type": {
			"_source": {
				"enabled": true
...
curl -XGET s01:9200/ex_index/_mapping?pretty=true
echo -e "\nAdding a point"
curl -XPUT s01:9200/ex_index/ex_type/1 -d '{
	"name": "Andy",
	"pin" : {
		"location": {
			"type": "point",
...
curl -XPUT s01:9200/ex_index/ex_type/2 -d '{
	"name": "Bobbi",
	"pin" : {
		"location": {
			"type": "point",
...
curl -XGET s01:9200/ex_index/ex_type/_search?pretty=true -d '{
